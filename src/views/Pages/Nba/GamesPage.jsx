...
class GamesPage extends React.Component {
  constructor(props) {
    ...
    const gameTypeInitSelectId = (props.nbaGameTypes.length > 0 && ("id" in props.nbaGameTypes[0])) ? props.nbaGameTypes[0].id : 1;
    ...
    this.state = {
      ...
      columns: [
        ...
        { title: 'Date', field: 'DateTime', type: 'datetime', render: data => moment(data.DateTime).format('DD-MM-YYYY HH:mm:ss'), editable: 'never' }
      ],
      detailsColumns: [
      ...
      data: props.nbaGames,
      sportId: props.nbaSportId,
      userGamesData: props.nbaUserGames,
      gameStyles: props.gameStyles,
      gameTypes: props.nbaGameTypes,
    ...
    if(this.state.selectedGameCnt) {
      try {
        const matchupData = {
          ...
          week: 0,
          game_key: rowData.GameID,
          score_id: 0,
          date: rowData.DateTime,
          game_type_category: rowData.GameTypeCategory
        }
        await axios.post('/nba/matchups', matchupData);
  ...
  refreshData = async () => {
    try {
      const response = await axios.get('/nba/matchups_in_current_season');
  ...
  clasifyGamesByType = () => {
    ...
    const categorizedGameData = this.state.data.map((item, i) => {
      ...
      switch(dayName) {
        case 'Sunday':
          sun.push({...item, GameTypeCategory: 'sun'});
        break;
        case 'Monday':
          mon.push({...item, GameTypeCategory: 'mon'});
        break;
        case 'Tuesday':
          tue.push({...item, GameTypeCategory: 'tue'});
        break;
        case 'Wednesday':
          wed.push({...item, GameTypeCategory: 'wed'});
        break;
        case 'Thursday':
          thu.push({...item, GameTypeCategory: 'thu'});
        break;
        case 'Friday':
          fri.push({...item, GameTypeCategory: 'fri'});
        break;
        case 'Saturday':
          ...
          <GridItem xs={12}>
            <Card>
              <CardHeader color="primary" icon>
                ...
                <h4 className={classes.cardIconTitle}>NBA</h4>
              </CardHeader>
              <CardBody>
                <GridContainer>
                          ...
                          detailPanel={rowData => {
                            return (
                              <MaterialTable
                                ...
                                data={query =>
                                  new Promise((resolve, reject) => {
                                    axios.get(`/nba/matchups/${rowData.id}`).then(function (response) {
